...
public:
    ...
    std::string message([[maybe_unused]] int condition /*unused*/) const override
    {
...
constexpr std::error_category &subprocess_category()
{
    return subprocess_category_impl;
}
...
public:
    ...
    {}
    subprocess_error(
        std::initializer_list<std::string> what_arg,
        int errc,
        const std::error_category &errcat = subprocess_category())
        : subprocess_error{
            [&what_arg] {
                ...
                std::copy(
                    what_arg.begin(), what_arg.end(), std::ostream_iterator<std::string>(ostr, " "));
                return ostr.str();
            }(),
            errc,
            errcat}
...
{
public:
    explicit usage_error(std::string_view what_arg)
        : subprocess_error{what_arg, -1}
    {}
};
...
public:
    ...
    [[nodiscard]] decltype(std::declval<wordexp_t>().we_wordv) argv() const &
    {
        return parsed_args_.we_wordv;
    }
...
inline descriptor_ptr_t<T> make_descriptor(Args &&...args)
{
...
{
public:
    explicit ovariable_descriptor(std::string &output_var)
        : output_{output_var}
    {
        linked_fd_ = &input_pipe_;
    }
...
{
public:
    explicit ivariable_descriptor(std::string input_data)
        : input_{std::move(input_data)}
    {
...
{
    static auto stderr_fd{make_descriptor<descriptor>(
        static_cast<int>(posix_util::standard_filenos::standard_error))};
    return stderr_fd;
};
...
inline void link(ipipe_descriptor &fd1, opipe_descriptor &fd2)
{
    if (fd1.linked_fd_ != nullptr or fd2.linked_fd_ != nullptr) {
        throw exceptions::usage_error{"You tried to link a file descriptor that is already linked "
...
{
public:
    explicit posix_process(std::string cmd)
        : cmd_{std::move(cmd)}
    {}
...
{
    auto process_fds = [](posix_util::posix_spawn_file_actions &action,
                          descriptor_ptr &fd,
    ...
    int pid{};
    if (int err{::posix_spawnp(&pid, sh.argv()[0], action.get(), nullptr, sh.argv(), nullptr)};
        err != 0) {
...
inline command &command::operator|(std::string other)
{
    return *this | command{std::move(other)};
}
...
inline command &&operator>(command &&cmd, descriptor_ptr fd)
{
    return std::move(cmd > std::move(fd));
}
...
inline command operator""_cmd(const char *cmd, size_t /*unused*/)
{
    return command{cmd};
}
} // namespace literals
